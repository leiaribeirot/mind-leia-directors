# Suggest Refactoring - Arcadia Developer Task
...
```bash
# Analyze single file
*suggest-refactoring arcadia-core/utils/complex-utility.js

# Analyze directory with specific patterns
*suggest-refactoring arcadia-core/agents --patterns extract_method,decompose_class --recursive

# Apply high-impact suggestions
*suggest-refactoring arcadia-core/utils --threshold 7 --apply-all

# Export suggestions for review
*suggest-refactoring . --recursive --export refactoring-report.json

# Dry run to see changes
*suggest-refactoring arcadia-core/agents/developer.md --apply ref-001 --dry-run
```
...
class SuggestRefactoringTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🔧 Arcadia Refactoring Analysis'));
  ...
  async loadSuggestion(suggestionId) {
    ...
    const cacheFile = path.join(this.rootPath, '.arcadia', 'refactoring', `${suggestionId}.json`);
